...
function Holidays({ products }: Props) {
    ...
    useEffect(() => {
        setLoading(true);
        setCatProducts(products.filter((product: Product) => product.category === 'holidays').sort((a: Product, b: Product) => (a.name).localeCompare(b.name)));
    }, [products])
